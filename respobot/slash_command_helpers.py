...
class SlashCommandHelpers:
                    ...
                    for series_info_tuple in series_info_tuples:
                        ...
                        for car_class_tuple in car_class_tuples:
                            ...
                            if car_class_season_id == season_id:
                                new_iracing_series_dict[season]['season_class_dict'][season_name].append(car_class_short_name)
    ...
    def get_member_list(self, ctx: discord.AutocompleteContext):
        return [member_string for member_string in self.member_name_list if member_string.lower().find(ctx.value.lower()) > -1 or ctx.value == '' or ctx.value is None]

    @classmethod
    def get_admin_member_list(self, ctx: discord.AutocompleteContext):
        return [member_string for member_string in self.admin_member_name_list if member_string.lower().find(ctx.value.lower()) > -1 or ctx.value == '' or ctx.value is None]

    @classmethod
    async def get_iracing_seasons(self, ctx: discord.AutocompleteContext):
        return [season for season in self.iracing_season_strings if season.lower().find(ctx.value.lower()) > -1 or ctx.value == '' or ctx.value is None]
    ...
    def get_all_series_list(self, ctx: discord.AutocompleteContext):
        ...
        series_keyword_list = [f"{series['series_name']} ({series['series_id']})" for series in self.iracing_series_dict['all_series']]

        return [series_keyword for series_keyword in series_keyword_list if (series_keyword.lower().find(ctx.value.lower()) > -1 or ctx.value == '' or ctx.value is None)]
