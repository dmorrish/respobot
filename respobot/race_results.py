...
async def get_race_results(bot: discord.Bot, db: BotDatabase, ir: IracingClient):
    ...
    try:
        ...
        logging.getLogger('respobot.bot').warning(f"During get_race_results the following exception was encountered: {exc}")
        await helpers.send_bot_failure_dm(bot, f"During get_race_results the following exception was encountered: {exc}")
        return
    ...
    for member_dict in member_dicts:
        ...
        try:
            races_list = await ir.search_results_new(cust_id=iracing_custid, finish_range_begin=start_low_str, finish_range_end=start_high_str)
        except ValueError:
        ...
        try:
            hosted_races_list = await ir.search_hosted_new(cust_id=iracing_custid, finish_range_begin=start_low_str, finish_range_end=start_high_str)
        ...
        for race in races_list:
            ...
            if race_found is False:
                logging.getLogger('respobot.bot').info(f"Attempting to add new subsession: {race['subsession_id']}")
                try:
                ...
                if laps_found is False and 'session_results' in new_race and len(new_race['session_results']) > 0:
                    ...
                    for session_result_dict in new_race['session_results']:
                        ...
                        try:
                            lap_dicts = await ir.lap_data_new(new_race['subsession_id'], session_result_dict['simsession_number'])
                        except Exception as exc:
                            logging.getLogger('respobot.bot').warning(
                        ...
                        try:
                            await db.add_laps(lap_dicts, new_race['subsession_id'], session_result_dict['simsession_number'])
                        except BotDatabaseError as exc:
                            logging.getLogger('respobot.bot').warning(
...
async def generate_race_report(bot: discord.Bot, db: BotDatabase, subsession_id: int, embed_type: str = 'auto'):
    try:
        ...
        if current_racing_week is None or current_racing_week < 0:
            current_racing_week = await stats.get_number_of_race_weeks(datetime.now(timezone.utc).isoformat().replace('+00:00', 'Z'))

        week_data_before = await stats.get_respo_champ_points(db, all_member_dicts, current_year, current_quarter, current_racing_week, subsession_to_ignore=subsession_id)
        ...
        for car_number in respo_car_numbers:
            ...
            # Cycle through the driver race results and if they are a Respo member, overwrite their display_name with their Respo member name
                    ...
                    if driver_race_result.irating_new >= constants.PLEB_LINE:
                        ...
                        if driver_race_result.irating_old < constants.PLEB_LINE:
                            if role_change_reason != "":
                                role_change_reason += "\n"  # Add a newline if this is not the first driver to cross the pleb line.
                            role_change_reason += driver_race_result.display_name + " has risen above the pleb line and proven themself worthy of the title God Amongst Men."
                    else:
                        await roles.demote_driver(helpers.fetch_guild(bot), member_dict['discord_id'])
                        if driver_race_result.irating_old >= constants.PLEB_LINE:
                            if role_change_reason != "":
                                role_change_reason += "\n"  # Add a newline if this is not the first driver to cross the pleb line.
                            role_change_reason += driver_race_result.display_name + " has dropped below the pleb line and has been banished from Mount Olypmus to carry out the rest of their days among the peasants."

                if env.SUPPRESS_RACE_RESULTS is False and role_change_reason:
                    await channel.send(role_change_reason)

            # Cycle through the driver heat results and if they are a Respo member, overwrite their display_name with their Respo member name
        ...
        logging.getLogger('respobot.bot').warning(f"During generate_race_report() the following exception was caught: {exc}")
        await helpers.send_bot_failure_dm(bot, f"During generate_race_report() the following exception was caught: {exc}\nTraceback: {traceback_string}")
...
async def generate_race_event_message(db: BotDatabase, car_results: subsession_summary.CarResultSummary):
    ...
    if car_results.disqualified:
        message = f"Hey, everyone! {he_she_they[0].upper()}{he_she_they[1:]} {was_were} disqualified from the race. <:KEKW:821408061960486992>"
    if car_results.did_not_finish:
        message = f"Positively delightful! {he_she_they[0].upper()}{he_she_they[1:]} DNFed from the race. <:KEKW:821408061960486992>"
    elif car_results.black_flag_laps is not None and len(car_results.black_flag_laps) > 0:
        message = f"Pathetic. {he_she_they[0].upper()}{he_she_they[1:]} {was_were} black flagged on lap {min(car_results.black_flag_laps)}. <:KEKW:821408061960486992>"
    elif car_results.green_flag_lap_num in car_results.tow_laps:
        message = f"I'm no pro or anything, but I'm pretty sure having to tow on the green flag lap isn't a good strategy."
        ...
        len([lap for lap in car_results.tow_laps if lap < car_results.green_flag_lap_num]) < 1 and len(car_results.tow_laps) > 0
        ...
        message = (
            ...
            f"{himself_herself_themself} on lap{'s' if len(self_spin_laps) > 1 else ''} {helpers.format_grammar_for_item_list(self_spin_laps)}."
        )
    ...
    elif car_results.race_finish_position_relative_to_car_num is not None and car_results.race_finish_position_relative_to_car_num > 9:
        message = f"Respect where respect is due. {he_she_they[0].upper()}{he_she_they[1:]} finished {car_results.race_finish_position_relative_to_car_num} positions better than {his_her_their} car number."
    elif len(car_results.contact_laps) > 2:
        message = f"You made contact with track objects on {car_results.contact_count} separate laps. At some point it's just vandalism."
        ...
        message = f"{he_she_they[0].upper()}{he_she_they[1:]} finished {int_laps_down} lap{'s' if int_laps_down > 1 else ''} down. Git gud, {boy_girl_people}."
        ...
        if car_results.close_finishers[0][1] > 0:
            message += f"but unfortunately {he_she_they} didn't have it in {him_her_them} and {he_she_they} lost by {car_results.close_finishers[0][1]/1000}s."
        else:
            message += f"and miraculously {he_she_they} came out ahead by {abs(car_results.close_finishers[0][1]/1000)}s."
    elif car_results.close_finishers is not None and len(car_results.close_finishers) > 1:
        ...
        if len(cars_ahead) == 0:
            ...
            message += (
                f"Car{'s' if len(cars_ahead) > 1 else ''} {helpers.format_grammar_for_item_list(cars_ahead)} came out ahead of {him_her_them} and "
                f"car{'s' if len(cars_behind) > 1 else ''} {helpers.format_grammar_for_item_list(cars_behind)} finished behind."
            )
...
async def send_results_embed(bot: discord.Bot, db: BotDatabase, channel: discord.TextChannel, car_results: subsession_summary.CarResultSummary):
    try:
        ...
        if multi_report is False:
            ...
            avatar_filepath = env.BOT_DIRECTORY + "media/respo_logo.png"
        ...
        with open(avatar_filepath, "rb") as f_avatar:
            ...
            if car_results.league_name is None:
                embedVar.add_field(name="Champ. Pts", value=str(car_results.driver_race_results[0].champ_points), inline=True)
                ...
                for driver_race_result in car_results.driver_race_results:
                    ...
                    if multi_report is False:
                        safety_rating_change = f"({((driver_race_result.license_sub_level_new - driver_race_result.license_sub_level_old) / 100):+.2f}, {driver_race_result.incidents}x)"
                    else:
                        safety_rating_change = f"{((driver_race_result.license_sub_level_new - driver_race_result.license_sub_level_old) / 100):+.2f}"

                    if multi_report is False:
                        embedVar.add_field(name=f"iRating", value=f"{driver_race_result.irating_new} ({irating_change:+d})", inline=False)
                        ...
                        if (driver_race_result.irating_new < 0) or (driver_race_result.laps_complete < 1):
                            ...
                            embedVar.add_field(
                                ...
                                value=f"{irating_change:+d} iR ({driver_race_result.irating_old}), {safety_rating_change} SR ({driver_race_result.incidents}x), {driver_race_result.laps_complete} Laps",
                                inline=False
                            )
            else:
                for driver_race_result in car_results.driver_race_results:
                    if multi_report is False:
                        embedVar.add_field(name=f"Incidents", value=f"{driver_race_result.incidents}x", inline=False)
                    else:
                        embedVar.add_field(name=driver_race_result.display_name, value=f"{driver_race_result.incidents}x", inline=False)
        ...
        logging.getLogger('respobot.bot').warning(f"During send_results_embed() the following exception was caught: {exc}")
        await helpers.send_bot_failure_dm(bot, f"During send_results_embed() the following exception was caught: {exc}")


async def send_results_embed_compact(bot: discord.bot, db: BotDatabase, channel: discord.TextChannel, car_results: subsession_summary.CarResultSummary):
    try:
        multi_report = len(car_results.driver_race_results) > 1
        ...
        with open(avatar_filepath, "rb") as f_avatar:
            ...
            result_field_name = "Result: P" + str(car_results.race_finish_position_in_class + 1) + " of " + str(car_results.cars_in_class) + pos_change_str
            ...
            if multi_report is True:
                ...
                for driver_race_result in car_results.driver_race_results:
                    ...
                    result_string += str(driver_race_result.incidents) + "x, " + str(driver_race_result.laps_complete) + " laps"
                    embedVar.add_field(name=driver_race_result.display_name, value=result_string, inline=False)
            else:
                result_string = ""
                if car_results.league_name is None:
                    result_string += str(car_results.driver_race_results[0].champ_points) + " pts"
                    irating_change = car_results.driver_race_results[0].irating_new - car_results.driver_race_results[0].irating_old
                    result_string += ", " + f"{irating_change:+d}" + " iR (" + str(car_results.driver_race_results[0].irating_new) + "), "
        ...
        logging.getLogger('respobot.bot').warning(f"During send_results_embed_compact() the following exception was caught: {exc}")
        await helpers.send_bot_failure_dm(bot, f"During send_results_embed_compact() the following exception was caught: {exc}")
        raise exc
