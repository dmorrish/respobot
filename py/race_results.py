...
async def get_hosted_results(bot: discord.Bot, db: BotDatabase, ir: IracingClient):
    return
...
async def process_race_result(bot: discord.Bot, db: BotDatabase, subsession_data: dict, embed_type: str = 'auto'):
    ...
    if 'subsession_id' not in subsession_data:
        log.logger_respobot.warning("iRacing returned subsession data without a subsession_id.")
        return
    ...
    if 'session_results' not in subsession_data or len(subsession_data['session_results']) < 1:
        log.logger_respobot.warning(f"iRacing returned data for subsession {subsession_data['subsession_id']} that was missing the session_results list.")
        return
    ...
    for results_dict in subsession_data['session_results']:
        if 'simsession_number' in results_dict and results_dict['simsession_number'] == 0:
            if 'simsession_type' in results_dict and results_dict['simsession_type'] != pyracingConstants.SimSessionType.race.value:
                log.logger_respobot.warning(f"The main event for subsession {subsession_data['subsession_id']} was not a race. Ignoring.")
                return
            if 'results' not in results_dict or len(results_dict['results']) < 1:
                log.logger_respobot.warning(f"iRacing returned no results list in the session_results for the main event for subsession {subsession_data['subsession_id']}")
                return
            race_results_list = results_dict['results']

    if race_results_list is None:
        log.logger_respobot.warning(f"No results list found in subsession {subsession_data['subsession_id']}")
    ...
    for car_number in respo_car_numbers:
        ...
        if member_dicts is None or len(member_dicts) < 1:
            log.logger_respobot.warning(f"Attempted to process race that returned no Respo members. Verify that the results table is populated for subsession {subsession_data['subsession_id']}")
            return
