...
class BotDatabase:
    ...
    async def init(self):
        ...
        if not await self._table_exists('subsessions'):
            print("creating table: subsessions")
        ...
        if not await self._table_exists('current_seasons'):
            print("creating table: current_seasons")
    ...
    #         log.logger_respobot.info("Connection to SQLite DB successful")
    #     except Error as e:
    #         log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} when running _create_connection() with path: {path}")
    ...
    async def execute_query(self, query, params=None):
        try:
            ...
            log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} when running execute_query() with query:\n{query}\nwith params:\n{params}")
            raise e
    ...
    async def execute_read_query(self, query, params=None):
        try:
            ...
            log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} when running execute_read_query() with query:\n{query}\nwith params:\n{params}")
            raise e
        ...
        for subsession_dict in subsession_dicts:
            ...
            try:
                ...
                if e.sqlite_errorcode == 1555:
                    log.logger_respobot.warning(f"Subsession already in database, ignoring.")
                    continue
                else:
                    subsessions = []
                    for subsession_dict in subsession_dicts:
                        subsessions.append(subsession_dict['subsession_id'])
                    log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} when trying to add subsession(s) {subsessions} to the subsessions table.")
            ...
            try:
                ...
                for subsession_dict in subsession_dicts:
                    subsessions.append(subsession_dict['subsession_id'])
                log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} when trying to add a result from subsession(s) {subsessions} to the results table.")
    ...
    async def get_race_summary(self, subsession_id: int):
        ...
        try:
            ...
            log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} during get_race_summary() for subsession {subsession_id}.")
        ...
        try:
            ...
            log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} during get_race_summary() when determining if subsession {subsession_id} was multiclass.")
    ...
    async def get_driver_result_summary(self, subsession_id: int, iracing_custid: int, simsession_number: int = 0, simsession_type: int = 6):
        ...
        try:
            ...
            log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} during get_driver_result_summary() for driver {iracing_custid} in subsession {subsession_id}.")
    ...
    async def is_season_in_seasons_table(self, season_id):
        ...
        try:
            ...
            log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} during is_season_in_season_table() for season_id {season_id}.")
        return len(result) > 0
    ...
    async def is_series_in_series_table(self, series_id):
        ...
        try:
            ...
            log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} during is_series_in_series_table() fpr series_id {series_id}.")
        return len(result) > 0
    ...
    async def is_car_class_in_season_car_classes_table(self, season_id, car_class_id):
        ...
        try:
            ...
            log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} during is_car_class_in_season_car_classes_table() for season_id {season_id} and car_class_id {car_class_id}.")
        return len(result) > 0
    ...
    async def is_season_in_season_dates_table(self, season_year, season_quarter):
        ...
        try:
            ...
            log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} during is_season_in_season_dates_table() for season_year {season_year} and season_quarter {season_quarter}.")
        return len(result) > 0
    ...
    async def is_car_class_car_in_db(self, car_class_id, car_id):
        ...
        try:
            ...
            log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} during is_car_class_car_in_db() for car_class_id {car_class_id} and car_id {car_id}.")
        return len(result) > 0
    ...
    async def update_current_seasons(self, current_season_dicts):
        ...
        try:
            ...
            log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} during update_current_seasons() when deleting the existing entries.")
        ...
        if len(new_current_season_parameters) > 0:
            try:
                ...
                log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} when trying to add season to the current_seasons table.")
    ...
    async def update_seasons(self, series_dicts):
        ...
        if len(new_season_parameters) > 0:
            try:
                ...
                log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} when trying to add new seasons to the seasons table.")

        if len(existing_season_parameters) > 0:
            try:
                await self.execute_query(dbq.UPDATE_SEASONS, params=existing_season_parameters)
            except Error as e:
                log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} when trying to update seasons in the seasons table.")

        if len(new_series_parameters) > 0:
            try:
                await self.execute_query(dbq.INSERT_SERIES, params=new_series_parameters)
            except Error as e:
                log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} when trying to add new series to the series table.")

        if len(existing_series_parameters) > 0:
            try:
                await self.execute_query(dbq.UPDATE_SERIES, params=existing_series_parameters)
            except Error as e:
                log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} when trying to update series in the series table.")

        if len(new_car_class_parameters) > 0:
            try:
                await self.execute_query(dbq.INSERT_SEASON_CAR_CLASSES, params=new_car_class_parameters)
            except Error as e:
                log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} when trying to add new car class to the season_car_classes table.")
        ...
        if len(existing_car_class_parameters) > 0:
            try:
                ...
                log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} when trying to update car class in the season_car_classes table.")
    ...
    async def update_season_dates(self, season_dicts):
        ...
        try:
            ...
            log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} when trying to add/update {season_dict['season_year']}s{season_dict['season_quarter']} in the season_dates table.")
    ...
    async def fetch_guild_member_ids(self):
        ...
        try:
            ...
            log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} during fetch_guild_member_ids().")
    ...
    async def fetch_iracing_cust_ids(self):
        ...
        try:
            ...
            log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} during fetch_iracing_cust_ids()")
    ...
    async def fetch_name(self, iracing_custid):
        ...
        try:
            ...
            log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} during fetch_name() for iracing_custid {iracing_custid}.")
    ...
    async def get_member_last_race_check(self, iracing_custid: int):
        ...
        try:
            ...
            log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} during get_member_last_race_check() for iracing_custid {iracing_custid}.")
    ...
    async def set_member_last_race_check(self, iracing_custid: int, last_race_check: datetime):
        ...
        try:
            ...
            log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} during set_member_last_race_check() for iracing_custid = {iracing_custid}.")
    ...
    async def get_member_ir(self, iracing_custid: int, category_id: int = pyracingConstants.Category.road.value):
        ...
        try:
            ...
            log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} during get_member_ir() for iracing_custid {iracing_custid}.")
    ...
    async def fetch_member_dict(self, iracing_custid: int = None, discord_id: int = None, first_name: str = None):
        ...
        try:
            ...
            log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} during fetch_member_dict() for {member}.")
    ...
    async def fetch_members_in_subsession(self, subsession_id: int, car_number: int = None):
        ...
        try:
            ...
            log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} during fetch_members_in_subsession() for subsession_id {subsession_id} and car_number {car_number}.")
    ...
    async def fetch_graph_colour(self, iracing_custid=None, discord_id=None):
        ...
        try:
            ...
            log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} during fetch_graph_colour() for iracing_custid {iracing_custid} and discord_id {discord_id}")

        if tuples is not None and len(tuples) > 0:
            colour_strings = tuples[0][0].split(",")

            if len(colour_strings) != 4:
                log.logger_respobot.warning(f"graph_colour incorrectly formatted in database for iracing_custid:{iracing_custid} discord_id:{discord_id}")
                return None
    ...
    async def set_graph_colour(self, graph_colour: list[int], iracing_custid: int = None, discord_id: int = None):
        ...
        if len(graph_colour) != 4:
            log.logger_respobot.warning("Graph colours must be lists of 4 integers: [r, g, b, a].")
            return
        ...
        try:
            ...
            log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} when trying to update graph colour to {graph_colour} for {member}.")
    ...
    async def get_latest_ir(self, iracing_custid: int = None, discord_id: int = None, first_name: str = None, category_id: int = pyracingConstants.Category.road.value):
        ...
        try:
            ...
            log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} during get_latest_ir() for {member}")
    ...
    async def get_ir_data(self, iracing_custid: int = None, discord_id: int = None, first_name: str = None, category_id: int = None):
        ...
        try:
            ...
            member = iracing_custid if iracing_custid is not None else discord_id if discord_id is not None else first_name
            log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} during get_ir_data() for member {member}.")
    ...
    async def update_car_classes(self, car_class_dicts):
        ...
        try:
            ...
            log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} when trying to add subsession {car_class_dict['subsession_id']} the subsessions table.")
    ...
    async def is_season_active(self):
        ...
        try:
            ...
            log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} during is_season_active().")
    ...
    async def get_current_iracing_week(self, series_id: int = 139):
        ...
        try:
            ...
            log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} during get_current_iracing_week() for series_id {series_id}.")
    ...
    async def get_season_basic_info(self, series_id: int = None, season_year: int = None, season_quarter: int = None):
        ...
        try:
            ...
            log.logger_respobot.error(
                f"The error '{e}' occurred with code {e.sqlite_errorcode} during get_season_basic_info() for "
    ...
    async def get_series_last_run_season(self, series_id: int):
        ...
        try:
            ...
            log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} during get_series_last_run_season() for series_id {series_id}.")
    ...
    async def get_series_id_from_season_name(self, season_name: str, season_year: int = None, season_quarter: int = None):
        ...
        for series_tuple in series_tuples:
            if series_tuple[1] == season_name:
                return series_tuple[0]
    ...
    async def get_season_car_classes(self, season_id: int = None, series_id: int = None, season_year: int = None, season_quarter: int = None):
        ...
        try:
            ...
            log.logger_respobot.error(
                f"The error '{e}' occurred with code {e.sqlite_errorcode} during get_season_car_classes() for "
    ...
    async def get_member_race_results(
        ...
        try:
            ...
            log.logger_respobot.error(
                f"The error '{e}' occurred with code {e.sqlite_errorcode} during get_member_race_results() for "
    ...
    async def get_member_series_raced(
        ...
        try:
            ...
            log.logger_respobot.error(
                f"The error '{e}' occurred with code {e.sqlite_errorcode} during get_member_series_raced() for "
    ...
    async def get_drivers_in_class(self, subsession_id, car_class_id, simsession_number: int = 0, simsession_type: int = 6):
        ...
        try:
            ...
            log.logger_respobot.error(
                f"The error '{e}' occurred with code {e.sqlite_errorcode} during get_drivers_in_class() for "
    ...
    async def get_season_dates(self):
        ...
        try:
            ...
            log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} during get_season_dates().")
    ...
    async def get_quotes(
        ...
        try:
            ...
            log.logger_respobot.error(
                f"The error '{e}' occurred with code {e.sqlite_errorcode} during get_quotes() for "
    ...
    async def is_quote_in_db(self, message_id: int):
        ...
        try:
            ...
            log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} during is_quote_in_db() for message_id {message_id}.")
    ...
    async def get_quote_leaderboard(self):
        ...
        try:
            ...
            log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} during get_quote_leaderboard().")
    ...
    async def get_quote_ids(self):
        ...
        try:
            ...
            log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} during get_quote_ids().")
    ...
    async def add_quote(self, quote_dict):
        ...
        try:
            ...
            if e.sqlite_errorcode == 1555:
                log.logger_respobot.warning(f"Quote already in database, ignoring.")
            else:
                log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} when trying to add a quote to the quotes table.")
    ...
    async def get_special_events(self, earliest_date):
        ...
        try:
            ...
            log.logger_respobot.error(f"The error '{e}' occurred with code {e.sqlite_errorcode} during get_special_events() for earliest_date {earliest_date}.")
